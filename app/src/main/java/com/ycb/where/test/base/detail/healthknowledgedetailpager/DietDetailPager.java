 ...
 * 健康饮食
 */
public class DietDetailPager extends BaseDetailPager {
    private RecyclerView mLvDetail;
    private SwipeRefreshLayout mSrl;
    private FrameLayout mFlPb;
    private ProgressBar mPb;
    private Button mReset;
    public DietDetailPager(Activity activity) {
        super(activity);
    }
    ...
    public void initData() {
        ...
        String healthy_diet = sp.getString("healthy_diet", "");
        if (TextUtils.isEmpty(healthy_diet)){
            url = Url.HEALTHY_DIET;
        }else{
            url = healthy_diet;
        }
        //创建请求对象
        MyGsonRequestUtils<HealthKnowledgeBean> socialHotspotBeanMyGsonRequestUtils = new MyGsonRequestUtils<>(Request
            ...
            public void onResponse(HealthKnowledgeBean response) {
                new HealthKnowledgeInitRecyclerView(response, mActivity,mLvDetail,mSrl,Url.HEALTHY_DIET_TYPE, mFlPb,"healthy_diet");
