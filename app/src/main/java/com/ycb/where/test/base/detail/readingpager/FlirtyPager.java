package com.ycb.where.test.base.detail.readingpager;
...
import com.ycb.where.test.init_recyclerview.FlirtyRecyclerView;
 ...
 * 美女
 */
public class FlirtyPager extends BaseDetailPager{
    private RecyclerView mLvDetail;
    private SwipeRefreshLayout mSrl;
    private FrameLayout mFlPb;
    private ProgressBar mPb;
    private Button mReset;
    public FlirtyPager(Activity activity) {
        super(activity);
    }
    ...
    public void initData() {
        ...
        String flirty = sp.getString("flirty", "");
        if (TextUtils.isEmpty(flirty)){
            url = Url.FLIRTY;
        }else{
            url = flirty;
        }
        //创建请求对象
        MyGsonRequestUtils<FlirtyBean> flirtyBeanMyGsonRequestUtils = new MyGsonRequestUtils<>(Request
            ...
            public void onResponse(FlirtyBean response) {
               new FlirtyRecyclerView(response, mActivity, mLvDetail,
                        mSrl, Url.FLIRTY_TYPE,
                        mFlPb,"flirty");
            }
        }, mActivity);
