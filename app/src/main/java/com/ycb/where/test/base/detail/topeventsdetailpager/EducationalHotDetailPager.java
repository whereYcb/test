 ...
 * 教育热点
 */
public class EducationalHotDetailPager extends BaseDetailPager {
    private RecyclerView mLvDetail;
    private SwipeRefreshLayout mSrl;
    private FrameLayout mFlPb;
    private ProgressBar mPb;
    private Button mReset;
    public EducationalHotDetailPager(Activity activity) {
        super(activity);
    }
    ...
    public void initData() {
        ...
        SharedPreferences sp = mActivity.getSharedPreferences("config", Context.MODE_PRIVATE);
         String hot_educational = sp.getString("hot_educational", "");
        if (hot_educational.isEmpty()){
            url = Url.HOT_EDUCATIONAL;
        }else{
            url = hot_educational;
        }
        System.out.println("MMMMMMMMMMMMMMMMMMMMMMMMMMMMM"+url);
        //创建请求对象
        MyGsonRequestUtils<SocialHotspotBean> socialHotspotBeanMyGsonRequestUtils = new MyGsonRequestUtils<>(Request
            ...
            public void onResponse(SocialHotspotBean response) {
                ...
                new TopEventsInitRecyclerView(response, mActivity,mLvDetail,mSrl,Url.HOT_EDUCATIONAL_TYPE,
                        mFlPb,"hot_educational");
