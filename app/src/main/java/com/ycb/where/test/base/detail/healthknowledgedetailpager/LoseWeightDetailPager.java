 ...
 */
public class LoseWeightDetailPager extends BaseDetailPager {
    private RecyclerView mLvDetail;
    private SwipeRefreshLayout mSrl;
    private FrameLayout mFlPb;
    private ProgressBar mPb;
    private Button mReset;
    public LoseWeightDetailPager(Activity activity) {
        super(activity);
    }
    ...
    public void initData() {
        ...
        String lose_weight = sp.getString("lose_weight", "");
        if (TextUtils.isEmpty(lose_weight)){
            url = Url.LOSE_WEIGHT;
        }else{
            url = lose_weight;
        }
        ...
        MyGsonRequestUtils<HealthKnowledgeBean> socialHotspotBeanMyGsonRequestUtils = new MyGsonRequestUtils<>(Request
            ...
            public void onResponse(HealthKnowledgeBean response) {
                new HealthKnowledgeInitRecyclerView(response, mActivity,mLvDetail,mSrl,Url.LOSE_WEIGHT_TYPE, mFlPb,"lose_weight");
            }
        }, mActivity);
        //创建队列
