 ...
 * 老人健康
 */
public class OldManDetailPager extends BaseDetailPager {
    private RecyclerView mLvDetail;
    private SwipeRefreshLayout mSrl;
    private FrameLayout mFlPb;
    private ProgressBar mPb;
    private Button mReset;
    public OldManDetailPager(Activity activity) {
        super(activity);
    }
    ...
    public void initData() {
        ...
        String elderly_health = sp.getString("elderly_health", "");
        if (TextUtils.isEmpty(elderly_health)){
            url = Url.ELDERLY_HEALTH;
        }else{
            url = elderly_health;
        }
        //创建请求对象
        MyGsonRequestUtils<HealthKnowledgeBean> socialHotspotBeanMyGsonRequestUtils = new MyGsonRequestUtils<>(Request
            ...
            public void onResponse(HealthKnowledgeBean response) {
                new HealthKnowledgeInitRecyclerView(response, mActivity,mLvDetail,mSrl,Url.ELDERLY_HEALTH_TYPE, mFlPb,"elderly_health");
            }
        }, mActivity);
        //创建队列
