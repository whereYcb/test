 ...
 * 财经热点
 */
public class FinanceHotspotDetailPager extends BaseDetailPager {
    private RecyclerView mLvDetail;
    private SwipeRefreshLayout mSrl;
   private FrameLayout mFlPb;
    private ProgressBar mPb;
    private Button mReset;
    public FinanceHotspotDetailPager(Activity activity) {
        super(activity);
    }
    ...
    public void initData() {
        ...
        SharedPreferences sp = mActivity.getSharedPreferences("config", Context.MODE_PRIVATE);
         String finance_hotspot = sp.getString("finance_hotspot", "");
        if (finance_hotspot.isEmpty()){
            url = Url.FINANCE_HOTSPOT;
        }else{
            url = finance_hotspot;
        }
        //创建请求对象
        MyGsonRequestUtils<SocialHotspotBean> socialHotspotBeanMyGsonRequestUtils = new MyGsonRequestUtils<>(Request
            ...
            public void onResponse(SocialHotspotBean response) {
               new TopEventsInitRecyclerView(response, mActivity,mLvDetail,mSrl,Url.FINANCE_HOTSPOT_TYPE, mFlPb,"hot_livelihood");
