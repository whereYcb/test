 ...
 * 心理健康
 */
public class PsychologyDetailPager extends BaseDetailPager {
    private RecyclerView mLvDetail;
    private SwipeRefreshLayout mSrl;
    private FrameLayout mFlPb;
    private ProgressBar mPb;
    private Button mReset;
    public PsychologyDetailPager(Activity activity) {
        super(activity);
    }
    ...
    public void initData() {
        ...
        String mental_health = sp.getString("mental_health", "");
        if (TextUtils.isEmpty(mental_health)){
            url = Url.MENTAL_HEALTH;
        }else{
            url = mental_health;
        }
        //创建请求对象
        MyGsonRequestUtils<HealthKnowledgeBean> socialHotspotBeanMyGsonRequestUtils = new MyGsonRequestUtils<>(Request
            ...
            public void onResponse(HealthKnowledgeBean response) {
                new HealthKnowledgeInitRecyclerView(response, mActivity,mLvDetail,mSrl,Url.MENTAL_HEALTH_TYPE, mFlPb,"mental_health");
            }
        }, mActivity);
        //创建队列
