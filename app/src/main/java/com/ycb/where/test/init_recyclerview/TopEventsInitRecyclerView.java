...
import com.ycb.where.test.adapter.SocialHotSpotAdapter;
 ...
 */
public class TopEventsInitRecyclerView {
    SocialHotspotBean mSocialHotspotBean;
    LinearLayoutManager mLayoutManager;
    RecyclerView mLvDetail;
    Context mActivity;
    List<SocialHotspotBean.tngouDetail> mDetails;
    SocialHotSpotAdapter adapter;
    SwipeRefreshLayout mSrl;
    FrameLayout flPb;
    int lastVisibleItem, i;
    int type;
    String fromPager;
    public TopEventsInitRecyclerView(SocialHotspotBean response, Context context, RecyclerView
            mLvDetail, SwipeRefreshLayout mSrl, int SOCIAL_HOTSPOT_TYPE, FrameLayout flPb,String fromPager) {
        this.mSocialHotspotBean = response;
    ...
    private void initRecyleView() {
        ...
        List<SocialHotspotBean.tngouDetail> list = mSocialHotspotBean.list;
        mDetails = new ArrayList<>();
        for (int i = list.size() - 1; i >= 0; i--) {
            mDetails.add(list.get(i));
        }
        adapter = new SocialHotSpotAdapter(mActivity,
                mDetails);
        mLvDetail.setAdapter(adapter);
        mLvDetail.setBackgroundColor(Color.WHITE);
        flPb.setVisibility(View.GONE);
        int size = mSocialHotspotBean.list.size();
        int refresh=-1;
    ...
    private void setRefreshListener() {
        adapter.setOnItemClickListener(new SocialHotSpotAdapter.OnRecyclerViewItemClickListener() {
            @Override
            public void onItemClick(View view, SocialHotspotBean.tngouDetail data) {
                Toast.makeText(mActivity, data.fromname, Toast.LENGTH_SHORT).show();
    ...
    private void refreshData() {
        ...
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                MyGsonRequestUtils<SocialHotspotBean> socialHotspotBeanMyGsonRequestUtils
                        = new MyGsonRequestUtils<>(Request.Method.GET, Url
                        .getRefreshUrl(mDetails.get
                        (0).id, type), new Response.ErrorListener() {
                    @Override
                    ...
                    public void onResponse(SocialHotspotBean response) {
                        List<SocialHotspotBean.tngouDetail> newdatas = response.list;
                        if (newdatas.isEmpty()) {
                            Toast.makeText(mActivity, "已经是最新的数据啦!", Toast.LENGTH_SHORT)
                                    .show();
                        } else {
                            ArrayList<SocialHotspotBean.tngouDetail> newLists = new
                                    ArrayList<>();
                            ...
                            //保存最新数据的地址
                           config.edit().putString(fromPager, Url.getRefreshUrl
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (id == -1) {
                    ...
                    MyGsonRequestUtils<SocialHotspotBean>
                            socialHotspotBeanMyGsonRequestUtils = new

                            MyGsonRequestUtils<>(Request.Method.GET, Url
                            .getTopEventsLoadingMoreUrl
...
//                                    error.printStackTrace();
                    ...
                    }, SocialHotspotBean.class, new Response
                            .Listener<SocialHotspotBean>() {
                        @Override
                        public void onResponse(SocialHotspotBean response) {
                            List<SocialHotspotBean.tngouDetail> moreData = response
                                    .list;
                            if (moreData.isEmpty()) {
                                Toast.makeText(mActivity, "已经没有更多数据啦!", Toast
                                        .LENGTH_SHORT).show();
                            } else {
                                List<SocialHotspotBean.tngouDetail> moreList = new
                                        ArrayList<SocialHotspotBean.tngouDetail>();
...
//        mSrl.setRefreshing(true);
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                long id = -1;
                if (i == 0) {
                    id = mDetails.get(mDetails.size()/2).id-300;
