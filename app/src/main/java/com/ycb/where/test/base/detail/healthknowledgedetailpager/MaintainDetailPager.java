 ...
 * 女性保养
 */
public class MaintainDetailPager extends BaseDetailPager {
    private RecyclerView mLvDetail;
    private SwipeRefreshLayout mSrl;
    private FrameLayout mFlPb;
    private ProgressBar mPb;
    private Button mReset;
    public MaintainDetailPager(Activity activity) {
        super(activity);
    }
    ...
    public void initData() {
        ...
        String women_care = sp.getString("women_care", "");
        if (TextUtils.isEmpty(women_care)){
            url = Url.WOMEN_CARE;
        }else{
            url = women_care;
        }
        ...
        MyGsonRequestUtils<HealthKnowledgeBean> socialHotspotBeanMyGsonRequestUtils = new MyGsonRequestUtils<>(Request
            ...
            public void onResponse(HealthKnowledgeBean response) {
                 new HealthKnowledgeInitRecyclerView(response, mActivity,mLvDetail,mSrl,Url.WOMEN_CARE_TYPE, mFlPb,"women_care");
            }
        }, mActivity);
        //创建队列
