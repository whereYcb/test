 ...
 * 娱乐热点
 */
public class EntertainmentHotDetailPager extends BaseDetailPager {
    private RecyclerView mLvDetail;
    private SwipeRefreshLayout mSrl;
    private FrameLayout mFlPb;
    private ProgressBar mPb;
    private Button mReset;
    public EntertainmentHotDetailPager(Activity activity) {
        super(activity);
    }
    ...
    public void initData() {
        ...
        SharedPreferences sp = mActivity.getSharedPreferences("config", Context.MODE_PRIVATE);
         String hot_entertainment = sp.getString("hot_entertainment", "");
        if (hot_entertainment.isEmpty()){
            url = Url.HOT_ENTERTAINMENT;
        }else{
            url = hot_entertainment;
        }
        //创建请求对象
        MyGsonRequestUtils<SocialHotspotBean> socialHotspotBeanMyGsonRequestUtils = new MyGsonRequestUtils<>(Request
            ...
            public void onResponse(SocialHotspotBean response) {
                 new TopEventsInitRecyclerView(response, mActivity,mLvDetail,mSrl,Url.HOT_ENTERTAINMENT_TYPE,
                        mFlPb,"hot_entertainment");
