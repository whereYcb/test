 ...
 * 医疗护理
 */
public class NursingDetailPager extends BaseDetailPager {
    private RecyclerView mLvDetail;
    private SwipeRefreshLayout mSrl;
    private FrameLayout mFlPb;
    private ProgressBar mPb;
    private Button mReset;
    public NursingDetailPager(Activity activity) {
        super(activity);
    }
    ...
    public View initViews() {
        ...
        public void initData() {
            ...
            String medical_care = sp.getString("medical_care", "");
            if (TextUtils.isEmpty(medical_care)){
                url = Url.MEDICAL_CARE;
            }else{
                url = medical_care;
            }
            //创建请求对象
            MyGsonRequestUtils<HealthKnowledgeBean> socialHotspotBeanMyGsonRequestUtils = new MyGsonRequestUtils<>(Request
                ...
                public void onResponse(HealthKnowledgeBean response) {
                    new HealthKnowledgeInitRecyclerView(response, mActivity,mLvDetail,mSrl,Url.MEDICAL_CARE_TYPE, mFlPb,"medical_care");
                }
            }, mActivity);
            //创建队列
