 ...
 * 孩子健康
 */
public class ChildrenDetailPager extends BaseDetailPager{
    private RecyclerView mLvDetail;
    private SwipeRefreshLayout mSrl;
    private FrameLayout mFlPb;
    private ProgressBar mPb;
    private Button mReset;
    public ChildrenDetailPager(Activity activity) {
        super(activity);
    }
    ...
    public void initData() {
        ...
        String child_health = sp.getString("child_health", "");
        if (TextUtils.isEmpty(child_health)){
            url = Url.CHILD_HEALTH;
        }else{
            url = child_health;
        }
        //创建请求对象
        MyGsonRequestUtils<HealthKnowledgeBean> socialHotspotBeanMyGsonRequestUtils = new MyGsonRequestUtils<>(Request
            ...
            public void onResponse(HealthKnowledgeBean response) {
               new HealthKnowledgeInitRecyclerView(response, mActivity,mLvDetail,mSrl,Url.CHILD_HEALTH_TYPE, mFlPb,"child_health");
